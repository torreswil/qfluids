...
class Rest extends CI_Controller {
    ...
    public function load_casing(){
        ...
        foreach ($cubiertas as $cubierta) {
           	echo '<tr id="this_casing_'.$cubierta['id'].'"><td><input type="text" style="width:100px;" disabled="" value="'.$cubierta['odfrac'].'" /></td><td><input type="text" style="width:100px;" disabled="" value="'.$cubierta['oddeci'].' '.$cubierta['od_unit'].'" /></td><td><input type="text" style="width:100px;" disabled="" value="'.$cubierta['iddeci'].' '.$cubierta['id_unit'].'" /></td><td><a href="#remove_casing" class="remove_casing_link" id="rm_casing_'.$cubierta['id'].'"><img src="/img/delete.png" /></a></td></tr>';
        }
	}
                ...
                foreach($materiales as $material) {
                        ...
                        if(empty($tmp[0]['id'])) {
                                ...
                                if(!$checked) {
                                        ...
                                        if($checked) {                                                
                                                $data['nombre_unidad'] = 'TN1';
                                                $data['prefijo'] = 'TN1';
                                                $data['equivalencia'] = 2005;
                                                $data['unidad_destino'] = 'lb';
                                        }
                                } 
                                if(!$checked) {
                                        ...
                                        if($checked) {                                                
                                                $data['nombre_unidad'] = 'TN2';
                                                $data['prefijo'] = 'TN2';
                                                $data['equivalencia'] = 4010;
                                                $data['unidad_destino'] = 'lb';
                                        }
                                }                                         
                                if(!$checked) {
                                        ...
                                        if($checked) {
                                                ...
                                                $data['nombre_unidad'] = (!empty($sx[1])) ? $unidad : 'CN';
                                                $data['prefijo'] = 'CN';
                                                $data['equivalencia'] = (!empty($sx[1])) ? $sx[1] : 1;
                                ...
                                if(!$checked) {
                                        ...
                                        if($checked) {
                                                ...
                                                $data['nombre_unidad'] = (!empty($sx[1])) ? $unidad : 'TM';
                                                $data['prefijo'] = 'TM';
                                                $data['equivalencia'] = (!empty($sx[1])) ? $sx[1] : 1;
                                                $data['unidad_destino'] = 'gal';
                                        }
                ...
                foreach($equipos as $equipo) {
                        ...
                        if(empty($tmp[0]['id'])) {
                                ...
                                if($checked) {
                                        ...
                                        $data['nombre_unidad'] = 'DIA';                                        
                                        $data['prefijo'] = 'DIA';                                        
                                        $data['equivalencia'] = (!empty($var[0])) ? $var[0] : 1;
                                        $data['unidad_destino'] = 'DIA';
                                } else {
                                        $var = explode('UNIDAD', $unidad);
                                        $data['nombre_unidad'] = 'UNIDAD';
                                        $data['prefijo'] = 'UNIDAD';
                                        $data['equivalencia'] = (!empty($var[0])) ? $var[0] : 1;
                                        $data['unidad_destino'] = 'UND';
                                }
                                $tmp = $this->Api->create('conversions_table', $data);
                        }
                        $unit = empty($tmp[0]['id']) ? $tmp : $tmp[0]['id'];
